import { ArrowLeft, BookOpen, Filter, Plus } from "lucide-react";
...
export function Context() {
    ...
    const getAllStudyItems = useCallback(async (contentId: number) => {
        try {
    ...
    const getContent = useCallback(async () => {
        ...
        try {
            ...
            const discipline = await get_content(parsedContentId, parsedId);
            ...
            getAllStudyItems(parsedContentId);
    ...
    }, [id, contentid, getAllStudyItems]);

    useEffect(() => {
        getContent();

        eventBus.on("content_item:updated", getContent);

        return () => {
            eventBus.off("content_item:updated", getContent);
        };
    }, [getContent]);
    ...
    return (
        <div className="space-y-6">
            ...
            >
                <div className="rounded-full group-hover:bg-blue-50 transition-colors">
                    <ArrowLeft size={20} />
                </div>
                <span className="font-medium">Voltar para a disciplina</span>
            </button>
